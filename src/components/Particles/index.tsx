...
const ParticlesWrapper: React.FC<ParticleProps> = ({ size, lineOpacity, mobileStars, desktopStars }) => {
    ...
    return (
        <ParticleMemo particleNum={particleNum} size={size} lineOpacity={lineOpacity} />
    )
}
//create special memo just to avoid rerendering 
//prevent re rendering using memo
const ParticleMemo: React.FC<ParticleMemoProps> = memo(function ParticleMemo({ size, lineOpacity, particleNum }) {
    return (
        // @ts-ignore
        <Particles
            params={{
            ...
            onScroll={(e: React.MouseEvent<HTMLCanvasElement>) => { e.preventDefault() }}
        />)
})
