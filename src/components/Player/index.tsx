...
const Player: React.FC<PlayerProps> = ({ theme }) => {
    ...
    const handleVolume = (e: React.ChangeEvent<HTMLInputElement>, val: number) => {
        e.preventDefault()
            ...
            <ClickAwayListener onClickAway={handleTooltip}>
                ...
                >
                    <div className={`${classes.iconBox}`} onClick={muteVolume}>
                        ...
                        {(volume > 0 && volume <= 60) && <VolumeDownRoundedIcon className={classes.icon} />}
                        {(volume > 60) && <VolumeUpRoundedIcon className={classes.icon} />}
                    </div>
                </Tooltip>
